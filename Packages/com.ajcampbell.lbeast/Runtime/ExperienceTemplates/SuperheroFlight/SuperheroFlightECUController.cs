...
namespace LBEAST.ExperienceTemplates.SuperheroFlight
{
    ...
    public class SuperheroFlightECUController : MonoBehaviour
    {
        ...
        private LBEASTUDPTransport udpTransport;
        ...
        void Update()
        {
            ...
            if (bECUConnected)
            {
                    ...
                    currentTime - lastTelemetryTime > connectionTimeout)
                {
                    // Connection lost
                    bECUConnected = false;
                    Debug.LogWarning("[LBEAST] SuperheroFlightECU: Connection timeout");
                }
            }
        }
        ...
        public bool InitializeECU(string ipAddress, int port)
        {
            ...
            {
                Debug.LogError($"[LBEAST] SuperheroFlightECU: Failed to initialize UDP connection to {ecuIPAddress}:{ecuPort}");
                return false;
            }
            ...
            bECUConnected = true;
            Debug.Log($"[LBEAST] SuperheroFlightECU: Connected to {ecuIPAddress}:{ecuPort}");
            return true;
        }
        ...
        private void OnBytesReceived(int channel, byte[] data)
        {
            ...
            if (channel == 310)
            {
                ...
                {
                    Debug.LogWarning($"[LBEAST] SuperheroFlightECU: Invalid winch state packet size ({data.Length} bytes, expected {Marshal.SizeOf<SuperheroFlightDualWinchState>()})");
                }
            }
            else if (channel == 311)
            {
                ...
                {
                    Debug.LogWarning($"[LBEAST] SuperheroFlightECU: Invalid telemetry packet size ({data.Length} bytes, expected {Marshal.SizeOf<SuperheroFlightTelemetry>()})");
                }
            }
        }
    }
