...
namespace LBEAST.ExperienceTemplates.SuperheroFlight
{
    ...
    public class FlightHandsController : MonoBehaviour
    {
        ...
        private LBEASTHandGestureRecognizer handGestureRecognizer;
        private LBEASTTrackingSystem trackingSystem;
        ...
        void Start()
        {
            ...
            handGestureRecognizer = GetComponent<LBEASTHandGestureRecognizer>();
            if (handGestureRecognizer == null)
            {
                handGestureRecognizer = gameObject.AddComponent<LBEASTHandGestureRecognizer>();
            }

            // Get or create tracking system
            trackingSystem = GetComponent<LBEASTTrackingSystem>();
            if (trackingSystem == null)
            {
                trackingSystem = gameObject.AddComponent<LBEASTTrackingSystem>();
            }
        ...
        public bool InitializeGestureController()
        {
            ...
            {
                Debug.LogWarning("[LBEAST] FlightHandsController: Tracking system not available - using fallback methods");
            }

            // Initialize hand gesture recognizer
            if (!handGestureRecognizer.InitializeRecognizer())
            {
                Debug.LogWarning("[LBEAST] FlightHandsController: Hand tracker not available - hand tracking will use fallback methods");
            }

            Debug.Log("[LBEAST] FlightHandsController: Initialized");
            return true;
        }
        ...
        {
            // Use Unity's native hand tracking APIs via LBEASTHandGestureRecognizer
