...
namespace LBEAST.ExperienceTemplates
{
    ...
    /// </summary>
    [AddComponentMenu("LBEAST/Experience Templates/Escape Room Experience")]
    public class EscapeRoomExperience : LBEASTExperienceBase
    {
        ...
        protected override bool InitializeExperienceImpl()
        {
            ...
            if (narrativeStateMachine != null && useNarrativeStateMachine)
            {
                ...
                Debug.Log($"[LBEAST] EscapeRoomExperience: Narrative state machine initialized with {defaultStates.Count} states");
            }

            Debug.Log("[LBEAST] EscapeRoomExperience: Initialization complete");
            return true;
        }
        ...
        private void InitializeEmbeddedDevices()
        {
            ...
            if (doorController != null)
            {
                ...
                {
                    Debug.Log($"[LBEAST] EscapeRoomExperience: Door controller initialized at {doorConfig.deviceAddress}:{doorConfig.port}");
                }
                else
                {
                    Debug.LogWarning("[LBEAST] EscapeRoomExperience: Failed to initialize door controller");
                }
            }
            ...
            if (propController != null && numberOfProps > 0)
            {
                ...
                {
                    Debug.Log($"[LBEAST] EscapeRoomExperience: Prop controller initialized at {propConfig.deviceAddress}:{propConfig.port}");
                }
                else
                {
                    Debug.LogWarning("[LBEAST] EscapeRoomExperience: Failed to initialize prop controller");
                }
            }
        }
        ...
        public bool UnlockDoor(int doorIndex)
        {
            ...
            {
                Debug.LogWarning($"[LBEAST] EscapeRoomExperience: Invalid door index {doorIndex}");
                return false;
            }

            if (doorController == null || !doorController.IsDeviceConnected())
            {
                Debug.LogWarning("[LBEAST] EscapeRoomExperience: Door controller not connected");
                return false;
            }
            ...
            Debug.Log($"[LBEAST] EscapeRoomExperience: Unlock command sent to door {doorIndex}");
            return true;
        }
        ...
        public bool LockDoor(int doorIndex)
        {
            ...
            Debug.Log($"[LBEAST] EscapeRoomExperience: Lock command sent to door {doorIndex}");
            return true;
        }
        ...
        public bool TriggerPropAction(int propIndex, float actionValue = 1.0f)
        {
            ...
            {
                Debug.LogWarning($"[LBEAST] EscapeRoomExperience: Invalid prop index {propIndex}");
                return false;
            }

            if (propController == null || !propController.IsDeviceConnected())
            {
                Debug.LogWarning("[LBEAST] EscapeRoomExperience: Prop controller not connected");
                return false;
            }
            ...
            Debug.Log($"[LBEAST] EscapeRoomExperience: Prop action triggered on prop {propIndex} (value: {actionValue:F2})");
            return true;
        }
        ...
        protected override void OnNarrativeStateChanged(string oldState, string newState, int newStateIndex)
        {
            ...
            Debug.Log($"[LBEAST] EscapeRoomExperience: Narrative state changed from {oldState} to {newState}");
            ...
            if (doorIndex >= 0 && doorIndex < numberOfDoors)
            {
                ...
                Debug.Log($"[LBEAST] EscapeRoomExperience: Automatically unlocked door {doorIndex} for state {newState}");
            }
        }
        ...
        private void OnDoorStateChanged(int channel, bool isUnlocked)
        {
            if (channel >= 0 && channel < doorUnlockStates.Length)
            {
                ...
                Debug.Log($"[LBEAST] EscapeRoomExperience: Door {channel} state changed to {(isUnlocked ? "UNLOCKED" : "LOCKED")}");
                ...
                if (isUnlocked && !wasUnlocked)
                {
                    ...
                    Debug.Log($"[LBEAST] EscapeRoomExperience: Door {channel} unlock confirmed by firmware");
                }
            }
        }
        ...
        private void OnPropSensorValue(int channel, float value)
        {
            if (channel >= 0 && channel < propSensorValues.Length)
            {
                ...
                Debug.Log($"[LBEAST] EscapeRoomExperience: Prop {channel} sensor value: {value:F3}");
            }
        }
    }
}
