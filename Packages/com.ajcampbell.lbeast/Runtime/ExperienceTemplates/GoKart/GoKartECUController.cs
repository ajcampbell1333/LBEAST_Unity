...
namespace LBEAST.ExperienceTemplates.GoKart
{
    ...
    public class GoKartECUController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (bECUConnected)
            {
                    ...
                    currentTime - lastThrottleFeedbackTime > connectionTimeout)
                {
                    // Connection lost
                    bECUConnected = false;
                    Debug.LogWarning("[LBEAST] GoKartECU: Connection timeout");
                }
            }
        }
        ...
        public bool InitializeECU(string ipAddress, int port = 8888)
        {
            ...
            {
                Debug.LogError($"[LBEAST] GoKartECU: Failed to initialize UDP connection to {ecuIPAddress}:{ecuPort}");
                return false;
            }
            ...
            bECUConnected = true;
            Debug.Log($"[LBEAST] GoKartECU: Connected to {ecuIPAddress}:{ecuPort}");
            return true;
        }
