...
namespace LBEAST.ProLighting
{
    ...
    {
        public event Action<LBEASTArtNetNode> OnNodeDiscovered;
        ...
        private readonly Dictionary<string, LBEASTArtNetNode> discoveredNodes = new Dictionary<string, LBEASTArtNetNode>();

        public bool IsConnected => transport != null && transport.IsConnected;

        public Dictionary<string, LBEASTArtNetNode> GetDiscoveredArtNetNodes() => discoveredNodes;

        public List<LBEASTArtNetNode> GetNodes() => discoveredNodes.Values.ToList();
        ...
        bool ParseArtPollReply(byte[] packetData, ref LBEASTArtNetNode node)
        {
