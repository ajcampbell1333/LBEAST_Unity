...
namespace LBEAST.ProLighting
{
    ...
    /// </summary>
    [AddComponentMenu("LBEAST/ProLighting Controller")]
    public class ProLightingController : MonoBehaviour
    {
        [Tooltip("ProLighting configuration")]
        public LBEASTProLightingConfig Config = new LBEASTProLightingConfig();
        ...
        void Update()
        {
            ...
            // Tick discovery
            if (artNetManager != null && Config.DMXMode == LBEASTDMXMode.ArtNet)
        ...
        public bool InitializeDMX(LBEASTProLightingConfig inConfig)
        {
            Config = inConfig;
            if (isInitialized)
            {
        ...
        {
            if (Config.DMXMode != LBEASTDMXMode.ArtNet || artNetManager == null)
            {
        ...
        public void DiscoverRDMFixtures()
        {
            ...
            foreach (var universe in universeBuffer.GetUniverses())
            {
                var discoveredFixtures = new List<LBEASTDiscoveredFixture>();
        ...
        public List<LBEASTArtNetNode> GetDiscoveredArtNetNodes()
        {
            if (artNetManager != null)
                return artNetManager.GetNodes();
            return new List<LBEASTArtNetNode>();
        }

        public List<LBEASTDiscoveredFixture> GetDiscoveredRDMFixtures() => RDMService != null ? RDMService.GetAll() : new List<LBEASTDiscoveredFixture>();
        ...
        public int AutoRegisterDiscoveredFixture(string rdmUID)
        {
            ...
            var fixture = new LBEASTDMXFixture
            {
        ...
        {
            if (Config.DMXMode == LBEASTDMXMode.USBDMX)
                return !string.IsNullOrEmpty(Config.COMPort);
            else if (Config.DMXMode == LBEASTDMXMode.ArtNet)
                return true;
            return false;
        }
        ...
        bool ReceiveRDMDiscoveryResponse(int universe, List<LBEASTDiscoveredFixture> outFixtures)
        {
            return false; // NOOP: Not yet implemented
        }
        ...
        bool ParseRDMResponse(byte[] responseData, ref LBEASTDiscoveredFixture fixture)
        {
            return false; // NOOP: Not yet implemented
        }
        ...
        void PollRDMFixtures(float deltaTime)
        {
            ...
            foreach (var pair in virtualFixtureToRDMUIDMap)
            {
                ...
                var universe = Config.DMXMode == LBEASTDMXMode.USBDMX ? 0 : fixture.Universe;
                if (SendRDMGetRequest(universe, rdmUID, 0x00F0, out var response))
                {
                    var discoveredFixture = new LBEASTDiscoveredFixture();
