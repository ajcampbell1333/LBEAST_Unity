...
namespace LBEAST.ProLighting
{
    ...
    {
        public static bool ValidateRegister(LBEASTDMXFixture candidate, FixtureRegistry registry, out string error)
        {
            error = string.Empty;
        ...
        static int RequiredChannels(LBEASTDMXFixture f)
        {
            return f.FixtureType switch
            {
                LBEASTDMXFixtureType.Dimmable => 1,
                LBEASTDMXFixtureType.RGB => 3,
                LBEASTDMXFixtureType.RGBW => 4,
                LBEASTDMXFixtureType.MovingHead => 8,
                LBEASTDMXFixtureType.Custom => f.CustomChannelMapping == null ? 1 : Mathf.Max(1, f.CustomChannelMapping.Count),
                _ => 1
            };
        }
    }
