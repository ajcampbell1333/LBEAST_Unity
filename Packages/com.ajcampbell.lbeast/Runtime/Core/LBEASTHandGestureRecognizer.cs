...
namespace LBEAST.Core
{
    ...
    public class LBEASTHandGestureRecognizer : MonoBehaviour
    {
        ...
        [Tooltip("Only process gestures for locally controlled players (multiplayer safety). When true (default): Only the local player's gestures are processed using OpenXR APIs. When false: All players' gestures are processed. For local players, uses OpenXR APIs. For remote players, uses replicated data from LBEASTVRPlayerReplicationComponent.")]
        ...
        private Pose GetHandJointPose(bool leftHand, XRHandJointID jointID)
        {
            ...
            if (!onlyProcessLocalPlayer || !ShouldProcessGestures())
            {
                ...
                LBEASTVRPlayerReplicationComponent replicationComp = GetVRReplicationComponent();
        ...
        /// </summary>
        private LBEASTVRPlayerReplicationComponent GetVRReplicationComponent()
        {
            return GetComponent<LBEASTVRPlayerReplicationComponent>();
        }
