...
namespace LBEAST.Core.Networking
{
    /// <summary>
    /// LBEAST UDP Transport Component
    /// 
    /// Provides channel-based UDP communication using the LBEAST binary protocol.
    ...
    public class LBEASTUDPTransport : MonoBehaviour
    {
        ...
        /// </summary>
        public bool InitializeUDPConnection(string ip, int port, string name = "LBEAST_UDP")
        {
            remoteIP = ip;
            remotePort = port;
            socketName = name;
        ...
        private void ParseBinaryPacket(byte[] data, int length)
        {
            ...
            {
                Debug.LogWarning("LBEASTUDPTransport: Invalid start marker");
                return;
            }
            ...
            if (length < 5)
            {
                Debug.LogWarning($"LBEASTUDPTransport: Packet too small ({length} bytes)");
                return;
            }
            ...
            {
                Debug.LogWarning("LBEASTUDPTransport: CRC mismatch");
                return;
            }
            ...
            switch (type)
            {
                ...
                default:
                    Debug.LogWarning($"LBEASTUDPTransport: Unknown data type ({type})");
                    break;
            }
        }
