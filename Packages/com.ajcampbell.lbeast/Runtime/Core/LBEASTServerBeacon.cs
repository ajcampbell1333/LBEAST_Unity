...
namespace LBEAST.Core
{
    ...
    /// </summary>
    public class LBEASTServerBeacon : MonoBehaviour
    {
        ...
        public bool StartServerBroadcast(ServerInfo info, int port = 7778, float interval = 2.0f)
        {
            if (isActive)
            {
                Debug.LogWarning("[LBEASTServerBeacon] Already active");
                return false;
            }
            ...
            try
            {
                ...
                Debug.Log($"[LBEASTServerBeacon] Server broadcast started on port {broadcastPort}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEASTServerBeacon] Failed to start server broadcast: {ex.Message}");
                return false;
            }
        }
        ...
        public bool StartClientDiscovery(int port = 7778, float timeoutDuration = 10.0f)
        {
            ...
            try
            {
                ...
                Debug.Log($"[LBEASTServerBeacon] Client discovery started, listening on port {broadcastPort}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEASTServerBeacon] Failed to start client discovery: {ex.Message}");
                return false;
            }
        }
        ...
        public void Stop()
        {
            ...
            Debug.Log("[LBEASTServerBeacon] Stopped");
        }
        ...
        private void SendBroadcast()
        {
            ...
            {
                Debug.LogError($"[LBEASTServerBeacon] Failed to send broadcast: {ex.Message}");
            }
        }
        ...
        private void OnDataReceived(IAsyncResult result)
        {
            ...
            {
                Debug.LogError($"[LBEASTServerBeacon] Error receiving data: {ex.Message}");
        ...
        private void ProcessDiscoveredServers(float deltaTime)
        {
            ...
            lock (serverLock)
            {
                ...
                foreach (string key in serversToRemove)
                {
                    ...
                    Debug.Log($"[LBEASTServerBeacon] Server lost: {lostServer.ServerName} ({lostServer.ServerIP})");
