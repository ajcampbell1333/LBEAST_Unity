...
namespace LBEAST.AIFacemask
{
    ...
    /// </summary>
    [AddComponentMenu("LBEAST/AI Face Controller")]
    public class AIFaceController : MonoBehaviour
    {
        ...
        public bool InitializeAIFace(AIFaceConfig inConfig)
        {
            if (isInitialized)
            {
                Debug.LogWarning("[LBEAST] AIFaceController: Already initialized");
                return true;
            }
            ...
            if (config.targetMesh == null)
            {
                Debug.LogError("[LBEAST] AIFaceController: No target mesh assigned");
                return false;
            }
            ...
            isInitialized = true;
            Debug.Log($"[LBEAST] AIFaceController: Initialized with {blendShapeIndices.Count} blend shapes (NVIDIA ACE receiver mode)");
            return true;
        }
        ...
        private void CacheBlendShapeIndices()
        {
            ...
            {
                Debug.LogWarning("[LBEAST] AIFaceController: No mesh found on SkinnedMeshRenderer");
                return;
            }
        ...
        public void ReceiveFacialAnimationData(FacialAnimationData animationData)
        {
            if (!isInitialized)
            {
                Debug.LogWarning("[LBEAST] AIFaceController: Cannot receive animation data - not initialized");
                return;
            }
        ...
        private void ApplyBlendShapesToMesh(Dictionary<string, float> blendShapeWeights)
        {
            ...
            foreach (var kvp in blendShapeWeights)
            {
                ...
                {
                    Debug.LogWarning($"[LBEAST] AIFaceController: Blend shape '{blendShapeName}' not found on mesh");
                }
            }
        }
