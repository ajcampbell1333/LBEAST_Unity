...
namespace LBEAST.LargeHaptics
{
    ...
    public class PlatformController4DOF : HapticPlatformController
    {
        ...
        public void SendTiltStruct(TiltState tiltState, int channel = 100)
        {
            ...
            {
                Debug.LogWarning("[LBEAST] 4DOFPlatformController: Cannot send tilt struct - not initialized or not connected");
                return;
            }

            // Send tilt as struct packet (default Channel 100)
            udpTransport.SendStruct(channel, tiltState);
            Debug.Log($"[LBEAST] 4DOFPlatformController: Sent tilt struct on Ch{channel} - Pitch: {tiltState.Pitch:F2}, Roll: {tiltState.Roll:F2}");
        }
        ...
        public void SendScissorLiftStruct(ScissorLiftState liftState, int channel = 101)
        {
            if (!IsInitialized() || !udpTransport.IsUDPConnected())
            {
                Debug.LogWarning("[LBEAST] 4DOFPlatformController: Cannot send scissor lift struct - not initialized or not connected");
                return;
            }

            // Send scissor lift state as struct packet (default Channel 101)
            udpTransport.SendStruct(channel, liftState);
            Debug.Log($"[LBEAST] 4DOFPlatformController: Sent scissor lift struct on Ch{channel} - Y: {liftState.TranslationY:F2}, Z: {liftState.TranslationZ:F2}");
        }
