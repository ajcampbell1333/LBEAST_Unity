    ...
    public class ArcadePaymentManager : MonoBehaviour
    {
        ...
        void StartWebhookServer()
        {
            ...
            try
            {
                ...
                Debug.Log($"[LBEAST.Payment] Webhook server started: {prefix}");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEAST.Payment] Failed to start webhook server: {ex.Message}");
        ...
        void ListenLoop()
        {
            while (isServerRunning && listener != null && listener.IsListening)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Debug.LogWarning($"[LBEAST.Payment] Error in webhook listener: {ex.Message}");
                }
            }
        }
        ...
        {
            Debug.Log($"[LBEAST.Payment:{config.provider}] Player {p.cardId} paid ${p.amount:F2}. New balance: ${p.newBalance:F2}");
            
            // TODO: Integrate with LBEAST experience system
            // Example: Find experience and start session
            // var experience = FindObjectOfType<LBEASTExperienceBase>();
        ...
        IEnumerator PollBalanceOnStartup()
        {
            ...
            {
                Debug.Log($"[LBEAST.Payment] Initial balance for {config.cardId}: ${balance:F2}");
            }));
        }
        ...
        public IEnumerator CheckBalance(string cardId, Action<float> callback = null)
        {
            ...
            {
                Debug.LogWarning("[LBEAST.Payment] CheckBalance called with empty cardId");
                callback?.Invoke(0f);
                yield break;
            }
            ...
            using (var www = UnityEngine.Networking.UnityWebRequest.Get(url))
            {
                ...
                {
                    Debug.LogWarning($"[LBEAST.Payment] CheckBalance failed: {www.error}");
                    callback?.Invoke(0f);
                }
            }
        }
        ...
        public IEnumerator AllocateTokens(string stationId, float amount, Action<bool> callback)
        {
            ...
            {
                Debug.LogWarning("[LBEAST.Payment] AllocateTokens called with empty stationId");
            ...
            using (var www = new UnityEngine.Networking.UnityWebRequest(url, "POST"))
            {
                ...
                if (!success)
                {
                    Debug.LogWarning($"[LBEAST.Payment] AllocateTokens failed: {www.error}");
                }
            }
        }
        ...
        float ExtractBalance(string json)
        {
            ...
            try
            {
                ...
                Debug.LogWarning($"[LBEAST.Payment] Could not extract {balanceField} from JSON: {json}");
                return 0f;
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[LBEAST.Payment] Failed to parse balance from JSON: {ex.Message}");
                return 0f;
            }
        }
        ...
        string GetLocalIPAddress()
        {
            ...
            {
                Debug.LogWarning($"[LBEAST.Payment] Failed to get local IP: {ex.Message}");
            }
            return "127.0.0.1";
        }
        ...
        void Update()
        {
            lock (lockObject)
            {
                while (actionQueue.Count > 0)
                {
                    ...
                    {
                        Debug.LogError($"[LBEAST.Payment] Error in main thread dispatcher: {ex.Message}");
                    }
                }
            }
        }
