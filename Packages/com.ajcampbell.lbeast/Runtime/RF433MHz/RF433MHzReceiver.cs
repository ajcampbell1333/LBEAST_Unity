...
namespace LBEAST.RF433MHz
{
    ...
    public class RF433MHzReceiver : MonoBehaviour
    {
        ...
        public bool InitializeReceiver(RF433MHzReceiverConfig inConfig)
        {
            ...
            if (receiverImpl != null && receiverImpl.IsConnected())
            {
                Debug.LogWarning("[LBEAST] RF433MHzReceiver: Already initialized");
                return false;
            }
            ...
            {
                Debug.LogError($"[LBEAST] RF433MHzReceiver: Failed to initialize receiver (Type: {config.ReceiverType}, Device: {config.USBDevicePath})");
                receiverImpl = null;
                return false;
            }

            Debug.Log($"[LBEAST] RF433MHzReceiver: Initialized (Type: {config.ReceiverType}, Device: {config.USBDevicePath})");
            return true;
        }
        ...
        public void ShutdownReceiver()
        {
            if (receiverImpl != null)
            {
                ...
                receiverImpl = null;
                Debug.Log("[LBEAST] RF433MHzReceiver: Shutdown");
            }
        }
        ...
        public bool AssignButtonFunction(int buttonCode, string functionName)
        {
            ...
            {
                Debug.LogWarning($"[LBEAST] RF433MHzReceiver: Cannot assign function to unlearned button (Code: {buttonCode})");
                return false;
            }

            if (string.IsNullOrEmpty(functionName))
            {
                Debug.LogWarning("[LBEAST] RF433MHzReceiver: Function name cannot be empty");
                return false;
            }
            ...
            Debug.Log($"[LBEAST] RF433MHzReceiver: Assigned function '{functionName}' to button {buttonCode}");
        ...
        public bool UnassignButtonFunction(int buttonCode)
        {
            ...
            Debug.Log($"[LBEAST] RF433MHzReceiver: Unassigned function from button {buttonCode}");
        ...
        public void ClearAllButtons()
        {
            ...
            Debug.Log("[LBEAST] RF433MHzReceiver: Cleared all learned buttons and mappings");
        ...
        public bool RemoveLearnedButton(int buttonCode)
        {
            ...
            Debug.Log($"[LBEAST] RF433MHzReceiver: Removed learned button {buttonCode}");
        ...
        public bool SaveButtonMappings(string customFilePath = "")
        {
            ...
            try
            {
                ...
                Debug.Log($"[LBEAST] RF433MHzReceiver: Saved {learnedButtons.Count} learned buttons and {buttonFunctionMappings.Count} mappings to {filePath}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEAST] RF433MHzReceiver: Failed to save button mappings to {filePath}: {ex.Message}");
                return false;
            }
        }
        ...
        public bool LoadButtonMappings(string customFilePath = "")
        {
            ...
            {
                Debug.Log($"[LBEAST] RF433MHzReceiver: Button mappings file not found at {filePath} (will create on first save)");
                return false;
            }
            ...
            try
            {
                ...
                if (jsonObject == null)
                {
                    Debug.LogError($"[LBEAST] RF433MHzReceiver: Failed to parse button mappings JSON from {filePath}");
                    return false;
                }
                ...
                {
                    Debug.Log($"[LBEAST] RF433MHzReceiver: Loaded {learnedButtons.Count} learned buttons and {buttonFunctionMappings.Count} mappings from {filePath} (saved: {lastSaved})");
                }
                else
                {
                    Debug.Log($"[LBEAST] RF433MHzReceiver: Loaded {learnedButtons.Count} learned buttons and {buttonFunctionMappings.Count} mappings from {filePath}");
                }
            ...
            {
                Debug.LogError($"[LBEAST] RF433MHzReceiver: Failed to load button mappings from {filePath}: {ex.Message}");
                return false;
            }
        }
        ...
        {
            // Use default path: Application.persistentDataPath/Config/LBEAST/RF433MHz_Buttons.json
            string configDir = Path.Combine(Application.persistentDataPath, "Config", "LBEAST");
        ...
        public void SetAutoSave(bool enable)
        {
            ...
            Debug.Log($"[LBEAST] RF433MHzReceiver: Auto-save {(enable ? "enabled" : "disabled")}");
        }
            ...
            foreach (var evt in events)
            {
                ...
                if (IsLearningModeActive())
                {
                    ...
                    if (!IsButtonLearned(evt.ButtonCode))
                    {
                        ...
                        Debug.Log($"[LBEAST] RF433MHzReceiver: Learned new button (Code: {evt.ButtonCode}, RollingCode: {evt.RollingCode})");
                    }
                }
