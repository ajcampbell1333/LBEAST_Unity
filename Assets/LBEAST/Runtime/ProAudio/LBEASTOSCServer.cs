...
namespace LBEAST.ProAudio
{
    ...
    /// </summary>
    public class LBEASTOSCServer : MonoBehaviour
    {
        ...
        public bool StartListening(int port)
        {
            if (isListening)
            {
                Debug.LogWarning("[LBEASTOSCServer] Already listening");
                return false;
            }
            ...
            try
            {
                ...
                Debug.Log($"[LBEASTOSCServer] Started listening on port {port}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEASTOSCServer] Failed to start listening: {ex.Message}");
                return false;
            }
        }
        ...
        public void StopListening()
        {
            ...
            Debug.Log("[LBEASTOSCServer] Stopped listening");
        }
        ...
        private void ReceiveLoop()
        {
            ...
            while (isListening && udpListener != null)
            {
                ...
                catch (SocketException ex)
                {
                    if (isListening)
                    {
                        Debug.LogWarning($"[LBEASTOSCServer] Receive error: {ex.Message}");
                    }
                    break;
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[LBEASTOSCServer] Receive loop error: {ex.Message}");
                    break;
                }
            }
        }
        ...
        private void ParseOSCMessage(byte[] data)
        {
            ...
            {
                Debug.LogWarning($"[LBEASTOSCServer] Failed to parse OSC message: {ex.Message}");
            }
        }
