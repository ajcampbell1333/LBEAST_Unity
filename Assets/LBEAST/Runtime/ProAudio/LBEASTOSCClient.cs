...
namespace LBEAST.ProAudio
{
    /// <summary>
    /// Lightweight OSC client implementation for Unity
    /// Uses UDP (same as LBEAST's other network modules) with OSC message encoding
    /// </summary>
    public class LBEASTOSCClient : MonoBehaviour
    {
        ...
        public bool Initialize(string serverIP, int serverPort = 10023)
        {
            if (isInitialized)
            {
                Debug.LogWarning("[LBEASTOSCClient] Already initialized");
                return false;
            }
            ...
            try
            {
                ...
                {
                    Debug.LogError($"[LBEASTOSCClient] Invalid server IP: {serverIP}");
                    return false;
                }
                ...
                Debug.Log($"[LBEASTOSCClient] Initialized (target: {serverIP}:{serverPort})");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEASTOSCClient] Failed to initialize: {ex.Message}");
                return false;
            }
        }
        ...
        public void SendOSCMessage(string address, float value)
        {
            ...
            {
                Debug.LogWarning("[LBEASTOSCClient] Not initialized");
                return;
            }
            ...
            {
                Debug.LogWarning($"[LBEASTOSCClient] Failed to send OSC message: {ex.Message}");
            }
        }
        ...
        public void Shutdown()
        {
            ...
            targetEndpoint = null;
            isInitialized = false;
            Debug.Log("[LBEASTOSCClient] Shutdown");
        }
