...
namespace LBEAST.ProAudio
{
    ...
    public class ProAudioController : MonoBehaviour
    {
        ...
        private LBEASTOSCClient oscClient;
        private LBEASTOSCServer oscServer; // For receiving OSC messages from physical board
        ...
        private void Awake()
        {
            oscClient = gameObject.AddComponent<LBEASTOSCClient>();
        }
        ...
        private void Update()
        {
            // OSC message processing happens in LBEASTOSCServer thread, dispatched to main thread
        ...
        public bool InitializeConsole(ProAudioConfig config)
        {
            ...
            if (oscClient.Initialize(Config.BoardIPAddress, Config.OSCPort))
            {
                ...
                if (Config.EnableReceive)
                {
                    ...
                    oscServer = gameObject.AddComponent<LBEASTOSCServer>();
                    if (oscServer != null)
                    {
