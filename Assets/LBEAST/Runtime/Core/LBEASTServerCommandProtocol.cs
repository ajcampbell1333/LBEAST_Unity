...
namespace LBEAST.Core
{
    ...
    /// </summary>
    public class LBEASTServerCommandProtocol : MonoBehaviour
    {
        ...
        public bool InitializeClient(string serverIP, int serverPort = 7779)
        {
            if (isActive)
            {
                Debug.LogWarning("[LBEASTServerCommandProtocol] Client already active");
                return false;
            }
            ...
            try
            {
                ...
                {
                    Debug.LogError($"[LBEASTServerCommandProtocol] Invalid server IP: {serverIP}");
                    return false;
                }
                ...
                Debug.Log($"[LBEASTServerCommandProtocol] Client initialized (target: {serverIP}:{serverPort})");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEASTServerCommandProtocol] Failed to initialize client: {ex.Message}");
                return false;
            }
        }
        ...
        public void ShutdownClient()
        {
            ...
            isActive = false;
            Debug.Log("[LBEASTServerCommandProtocol] Client shutdown");
        }
        ...
        public ServerResponseMessage SendCommand(ServerCommand command, string parameter = "")
        {
            ...
            try
            {
                ...
                Debug.Log($"[LBEASTServerCommandProtocol] Sent command {(int)command} (seq: {commandMsg.SequenceNumber})");
            ...
            {
                Debug.LogWarning($"[LBEASTServerCommandProtocol] Failed to send command: {ex.Message}");
        ...
        public void TickClient()
        {
            ...
            if (clientSocket.Available > 0)
            {
                try
                {
                    ...
                    if (response != null)
                        Debug.Log($"[LBEASTServerCommandProtocol] Received response: {response.Message}");
                }
                catch (Exception ex)
                {
                    Debug.LogWarning($"[LBEASTServerCommandProtocol] Error receiving response: {ex.Message}");
                }
            }
        }
        ...
        public bool StartListening()
        {
            if (isListening)
            {
                Debug.LogWarning("[LBEASTServerCommandProtocol] Already listening");
                return false;
            }
            ...
            try
            {
                ...
                isListening = true;
                Debug.Log($"[LBEASTServerCommandProtocol] Started listening on port {CommandPort}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[LBEASTServerCommandProtocol] Failed to start listening: {ex.Message}");
                return false;
            }
        }
        ...
        public void StopListening()
        {
            ...
            isListening = false;
            Debug.Log("[LBEASTServerCommandProtocol] Stopped listening");
        }
        ...
        private void OnDataReceived(IAsyncResult result)
        {
            ...
            try
            {
                ...
                if (command != null)
                {
                    ...
                    {
                        Debug.LogWarning($"[LBEASTServerCommandProtocol] Authentication failed for command {(int)command.Command} from {sender}");
                        SendResponse(new ServerResponseMessage(false, "Authentication failed"), sender);
                        return;
                    }

                    Debug.Log($"[LBEASTServerCommandProtocol] Received command {(int)command.Command} (seq: {command.SequenceNumber}) from {sender}");
                    lastSenderAddress = sender;
                    OnCommandReceived?.Invoke(command, sender);
                }
                else
                    Debug.LogWarning($"[LBEASTServerCommandProtocol] Failed to deserialize command from {sender}");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[LBEASTServerCommandProtocol] Error receiving data: {ex.Message}");
        ...
        public void SendResponse(ServerResponseMessage response, IPEndPoint clientAddress)
        {
            ...
            try
            {
                ...
                Debug.Log($"[LBEASTServerCommandProtocol] Sent response to {clientAddress}");
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[LBEASTServerCommandProtocol] Failed to send response to {clientAddress}: {ex.Message}");
            }
        }
