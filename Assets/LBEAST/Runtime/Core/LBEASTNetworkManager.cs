...
{
    /// <summary>
    /// Manages LAN multiplayer networking for LBEAST experiences
    /// Uses Unity NetCode for GameObjects (NGO) for reliable local network play
    /// </summary>
    public class LBEASTNetworkManager : MonoBehaviour
    {
        ...
        public bool InitializeNetwork()
        {
            ...
            {
                isInitialized = true;
                Debug.Log($"[LBEAST] Network manager initialized - Max players: {maxPlayers}");
                return true;
            }
        ...
        public bool StartHost()
        {
            ...
            {
                Debug.Log($"[LBEAST] Started as host on port {port}");
                return true;
            }

            Debug.LogError("[LBEAST] Failed to start host");
            return false;
        }
        ...
        public bool StartServer()
        {
            ...
            {
                Debug.Log($"[LBEAST] Started as server on port {port}");
                return true;
            }

            Debug.LogError("[LBEAST] Failed to start server");
            return false;
        }
        ...
        public bool StartClient()
        {
            ...
            {
                Debug.Log($"[LBEAST] Started as client, connecting to {ipAddress}:{port}");
                return true;
            }

            Debug.LogError("[LBEAST] Failed to start client");
            return false;
        }
        ...
        public void Shutdown()
        {
            if (networkManager != null)
            {
                ...
                Debug.Log("[LBEAST] Network shutdown");
            }
        }
