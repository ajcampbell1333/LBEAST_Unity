#!/bin/bash
# LBEAST Version Release Script
# Updates package.json and creates a git tag for the release

set -e

echo "========================================"
echo "LBEAST Version Release Helper"
echo "========================================"
echo
...
if command -v jq &> /dev/null; then
    ...
    jq ".version = \"$VERSION\"" Packages/com.ajcampbell.lbeast/package.json > Packages/com.ajcampbell.lbeast/package.json.tmp
    mv Packages/com.ajcampbell.lbeast/package.json.tmp Packages/com.ajcampbell.lbeast/package.json
else
    # Fallback to sed
    sed -i.bak "s/\"version\": \".*\"/\"version\": \"$VERSION\"/" Packages/com.ajcampbell.lbeast/package.json
    rm Packages/com.ajcampbell.lbeast/package.json.bak 2>/dev/null || true
fi
echo "  [OK] package.json updated"

echo
echo "Step 2: Staging package.json..."
git add Packages/com.ajcampbell.lbeast/package.json
