@echo off
REM LBEAST Unity Dedicated Server Launcher
REM Copyright (c) 2025 AJ Campbell. Licensed under the MIT License.

echo ========================================
echo LBEAST Unity Dedicated Server Launcher
echo ========================================
echo.

REM Default configuration
set EXPERIENCE_TYPE=AIFacemask
set SCENE_NAME=LBEASTScene
set PORT=7777
set MAX_PLAYERS=4
...
:start_server
echo Starting LBEAST Unity Dedicated Server...
...
set SERVER_PATH="%~dp0Builds\Server\LBEAST_UnityServer.exe"
...
start "LBEAST Unity Dedicated Server" %SERVER_PATH% -batchmode -nographics -port %PORT% -scene %SCENE_NAME% -experienceType %EXPERIENCE_TYPE% -maxPlayers %MAX_PLAYERS% -logFile ServerLog.txt
